...
lockfilecontent=[a1 a2 datestr(now)];
lockfilecontent=lockfilecontent(double(lockfilecontent)~=10);
lockfilecontent=lockfilecontent(double(lockfilecontent)~=32);
...
if exist([pwd '/datfiles/lock.g'])
  ...
  try
    dlmwrite('datfiles/lock.g',lockfilecontent,'');
    fid=fopen('datfiles/lock.g','r');
    lock=textscan(fid,'%s');
    fclose(fid);
    if strcmp(lockfilecontent,lock{1})
...
if exist([pwd '/datfiles/lock.Bfield'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.Bfield',lockfilecontent,'')
...
if exist([pwd '/datfiles/lock.gravity'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.gravity',lockfilecontent,'')
...
if exist([pwd '/datfiles/lock.Efield'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.Efield',lockfilecontent,'')
    ...
    try
      ...
      if length(numbers)>0 & ~(Nprocs<Nprocsref)
        ...
        if EfieldInExistence
          %load('Efield.mat')
          mEfile=matfile([pwd '/Efield.mat']);
          hightimes=[];
          for ii=1:length(numbers)
            hightimes(ii)=str2num(dd(numbers(ii)).name(16:22));
          end
          maxtimeinfile=mEfile.timestepsEfield(1,end);
          newnumbers=numbers(hightimes>maxtimeinfile);
        ...
        if EfieldInExistence
          ...
          llf=length(mEfile.timestepsEfield);
          lll=length(timestepsEfield);
          if lll>0
            mEfile.Properties.Writable=true;
            mEfile.timestepsEfield = ...
                [mEfile.timestepsEfield timestepsEfield];
            mEfile.Efieldmatrix(llf+1:llf+lll,:)=Efieldmatrix;
          end
...
if exist([pwd '/datfiles/lock.current'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.current',lockfilecontent,'')
...
%%%
      ...
      if length(numbers)>0 & ~(Nprocs<Nprocsref)
        ...
        if currentInExistence
          %load('current.mat')
          mcurfile=matfile([pwd '/current.mat']);
          hightimes=[];
          for ii=1:length(numbers)
            hightimes(ii)=str2num(dd(numbers(ii)).name(17:23));
          end
          maxtimeinfile=mcurfile.timestepscurrent(1,end);
          newnumbers=numbers(hightimes>maxtimeinfile);
          for ii=1:length(newnumbers)
            I = [];
            for jj = 0:Nprocs-1
...
%%%
        if currentInExistence
          ...
          llf=length(mcurfile.timestepscurrent);
          lll=length(timestepscurrent);
          if lll>0
            mcurfile.Properties.Writable=true;
            mcurfile.timestepscurrent = ...
                [mcurfile.timestepscurrent timestepscurrent];
            mcurfile.currentmatrix(llf+1:llf+lll,:)=currentmatrix;
          end
...
if exist([pwd '/datfiles/lock.density'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.density',lockfilecontent,'')
...
%%%%
      ...
      if length(numbers)>0 & ~(Nprocs<Nprocsref)
        ...
        if densityInExistence
          mdensfile=matfile([pwd '/density.mat']);
          hightimes=[];
          for ii=1:length(numbers)
            hightimes(ii)=str2num(dd(numbers(ii)).name(17:23));
          end
          maxtimeinfile=mdensfile.timestepsdensity(1,end);
          newnumbers=numbers(hightimes>maxtimeinfile);
          for ii=1:length(newnumbers)
            Dens = [];
            for jj = 0:Nprocs-1
...
%%%%
        if densityInExistence
          ...
          llf=length(mdensfile.timestepsdensity);
          lll=length(timestepsdensity);
          if lll>0
            mdensfile.Properties.Writable=true;
            mdensfile.timestepsdensity = ...
                [mdensfile.timestepsdensity timestepsdensity];
            mdensfile.densitymatrix(:,:,llf+1:llf+lll)=densitymatrix;
          end
...
if exist([pwd '/datfiles/lock.fzvzmu'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.fzvzmu',lockfilecontent,'')
...
if exist([pwd '/datfiles/lock.fzvz'])
  ...
  all_is_fine=logical(0);
  try
    dlmwrite('datfiles/lock.fzvz',lockfilecontent,'')
